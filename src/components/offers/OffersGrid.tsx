    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {offers.map((offer) => {
        ...
        return (
          <div
            key={offer.id}
            className={`bg-[#0A0F1C] border rounded-xl overflow-hidden transition-all duration-150 flex flex-col justify-between group relative ${
              isSelected
                ? 'border-[#1E5EFF] shadow-md shadow-[#1E5EFF]/10 bg-[#0E172E]'
                : 'border-[#162340] hover:border-[#22375F] hover:bg-[#0C1426]'
            }`}
          >
            ...
            <div className="p-3 space-y-3">
              <div className="flex gap-3 items-start">
                <div
                  ...
                  className="w-16 h-16 rounded-lg bg-[#070C18] border border-[#162340] overflow-hidden shrink-0 cursor-pointer hover:border-[#00C2FF] transition-colors"
                >
                  <img
                    src={offer.imageUrl}
                    alt={offer.name}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="text-[11px] text-[#8E9BAE] block truncate">
                    {offer.category} • {offer.storeName}
                  </span>
                  <h4
                    onClick={() => onViewOffer(offer)}
                    title={offer.name}
                    className="text-xs font-semibold text-[#E6E8EC] hover:text-[#00C2FF] cursor-pointer transition-colors line-clamp-2 leading-snug mt-0.5"
                  >
                    {offer.name}
                  </h4>
                </div>
              ...
              {/* Price & Discount */}
              <div className="flex items-baseline justify-between pt-1 border-t border-[#131D33]">
                <div>
                ...
                <span className="px-1.5 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/25 text-emerald-400 font-mono-numeric font-semibold text-xs">
                  -{offer.discountPercentage}%
                </span>
              </div>

              {/* Metrics: Rating, Sales, Commission */}
              <div className="grid grid-cols-3 gap-1.5 p-2 bg-[#070C18] border border-[#14203B] rounded-lg text-center text-xs">
                <div>
                  <span className="text-[10px] text-[#8E9BAE] block">Avaliação</span>
                  <div className="flex items-center justify-center gap-0.5 font-mono-numeric font-semibold text-amber-300 text-xs mt-0.5">
                    <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                    <span>{offer.rating.toFixed(1).replace('.', ',')}</span>
                  </div>
                </div>
                <div>
                  <span className="text-[10px] text-[#8E9BAE] block">Vendas</span>
                  <span className="font-mono-numeric font-semibold text-[#C8D1DE] text-xs mt-0.5 block">
                    {offer.salesVolume}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] text-[#8E9BAE] block">Comissão</span>
                  <span className="font-mono-numeric font-semibold text-emerald-400 text-xs mt-0.5 block">
              ...
              {/* Deal Score Display */}
              <div className="flex items-center justify-between px-2.5 py-1.5 bg-[#0B1326] border border-[#182747] rounded-lg">
                <span className="text-xs text-[#8E9BAE] font-medium">
                  Deal Score
                </span>
            ...
            <div className="p-3 pt-0 flex items-center justify-between gap-2 relative">
              <div className="flex-1">
                ...
                {(offer.status === 'Publicada' ||
                  ...
                  <Button
                    ...
                    variant="outline"
                    onClick={() => onViewOffer(offer)}
                    leftIcon={<Eye className="w-3.5 h-3.5 text-[#8E9BAE]" />}
                ...
                {offer.status === 'Rejeitada' && (
                  <Button
                    ...
                    variant="ghost"
                    onClick={() => onApproveOffer(offer.id)}
                    className="w-full text-xs text-[#8E9BAE] hover:text-[#00C2FF]"
                  >
                    Reavaliar
                  </Button>
                )}
              ...
              {isMenuOpen && (
                <div
                  ref={menuRef}
                  className="absolute right-3 bottom-full mb-1 w-44 bg-[#0E172C] border border-[#1E3057] rounded-xl shadow-2xl shadow-black/80 py-1.5 z-30 text-left animate-in fade-in zoom-in-95 duration-100"
                >
                  <button
                    type="button"
                    onClick={() => {
