...
> = ({ automation, isOpen, onClose, onSave }) => {
  ...
  const renderCapabilityRow = (
    ...
    const stateUi =
      state === 'AVAILABLE' ? (
        <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-emerald-700 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/25">
          <CheckCircle2 className="w-3 h-3" />
          Disponível
        </span>
      ) : state === 'REQUIRES_INTEGRATION' ? (
        <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-amber-700 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/25">
          <Globe className="w-3 h-3" />
          Requer integração
        </span>
      ) : state === 'REQUIRES_CONFIGURATION' ? (
        <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#2563EB] bg-[#2563EB]/15 px-2 py-0.5 rounded border border-[#2563EB]/30">
          <ShieldCheck className="w-3 h-3" />
          Requer configuração
        </span>
      ) : (
        <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#64748B] bg-[#EFF6FF] px-2 py-0.5 rounded border border-[#CBD5E1]">
          Não suportado
        </span>
      );

    return (
      <div className="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg bg-[#F8FAFC] border border-[#E2E8F0]">
        <div className="min-w-0">
          <p className="text-xs font-medium text-[#172033]">{label}</p>
  ...
  const renderIntegrationHint = () => {
    ...
    return (
      <div className="flex items-start gap-2.5 p-3 bg-[#EFF6FF] border border-[#BFDBFE] rounded-lg text-[11px] text-[#94A3B8]">
              ...
              <div className="flex flex-wrap gap-1.5">
                {ALL_MARKETPLACES.filter(
                  ...
                  <button
                    ...
                    className="px-2 py-0.5 rounded-md text-[10px] font-medium text-[#64748B] bg-[#F8FAFC] border border-dashed border-[#CBD5E1] hover:border-[#2563EB] hover:text-[#2563EB] transition-colors cursor-pointer"
                  >
                    + {mp}
                  </button>
                ))}
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {[
                  ...
                  <div
                    key={label}
                    className="p-2.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg"
                  >
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {a.sources.map((s) => (
                  <div
                    key={s.id}
                    className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-[#F8FAFC] border border-[#E2E8F0]"
                  >
                    <div className="min-w-0">
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded border transition-colors cursor-pointer shrink-0 ${
                        s.active === false
                          ? 'text-[#64748B] bg-[#EFF6FF] border-[#CBD5E1]'
        ...
        return (
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-[#F8FAFC] border border-[#E2E8F0]">
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-[#172033] truncate">
                    {a.sources[0]?.name}
                  </p>
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      {/* Drawer Panel */}
      <div className="absolute inset-y-0 right-0 max-w-xl w-full bg-[#FFFFFF] border-l border-[#E2E8F0] shadow-2xl flex flex-col z-10 animate-in slide-in-from-right duration-250">
        {/* Drawer Header */}
        <div className="p-5 border-b border-[#E2E8F0] bg-[#F8FAFC] flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-lg text-[#64748B] hover:text-[#172033] hover:bg-[#EFF6FF] transition-colors cursor-pointer shrink-0"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {/* Config fields */}
          <div className="p-4 bg-[#F1F5F9] border border-[#E2E8F0] rounded-xl space-y-4">
        ...
        {/* Drawer Footer */}
        <div className="p-4 border-t border-[#E2E8F0] bg-[#F8FAFC] flex flex-col gap-3">
          <div className="flex items-center gap-3">
            <Button
              variant="outline"
              size="sm"
              onClick={toggleStatus}
              className={`text-xs border-[#DCE3EC] ${
                isActive
            ...
            <Button
              ...
              className={
                isActive
                  ? 'text-xs'
                  : 'text-xs border-[#2563EB] text-[#2563EB]'
              }
            >
